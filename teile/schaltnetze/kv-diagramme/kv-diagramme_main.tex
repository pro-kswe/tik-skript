...
\begin{todolist}
\item Sie erstellen eine kompakte boolesche Formel in \ac{DNF} mithilfe eines \acs{KV}-Diagramms.
\end{todolist}
\end{minipage}
}
...
\item $T_1 = \neg E_1 \wedge \neg E_2$
\begin{itemize}
\item $\neg E_0 \notin T_1$: nur die \textbf{erste} Eins in diesem Block \say{besitzt} den Eingang $\neg E_0$
\item $E_0 \notin T_1$: nur die \textbf{zweite} Eins in diesem Block \say{besitzt} den Eingang $E_0$
\item $E_1 \notin T_1$ und $E_2 \notin T_1$: keine Eins \say{besitzt} einen dieser Eingänge in diesem Block 
\end{itemize}
...
Erstellen Sie \textbf{pro Übung} für \textbf{jeden Ausgang} die \textbf{kompakte boolesche Formel} in \ac{DNF} mithilfe des entsprechenden \ac{KV}-Diagramms. Stellen Sie die Blöcke im \ac{KV}-Diagramm \textbf{sauber} dar.

TODO: NOCH ZWEI ÜBUNGEN: 4 Zellen und 16 Zellen
