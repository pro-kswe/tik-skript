...
In \autoref{figure-logikgatter-uebung-oder} gibt es zwei Schalter, mit denen das Licht eingeschaltet werden kann. Das Licht wird erst ausgeschaltet, wenn beide Schalter ausgeschaltet sind. Tragen Sie das entsprechende Logikgatter für diese Situation ein.

\begin{figure}[htb]
\centering
\begin{minipage}{0.45\textwidth}
\centering
\includegraphics[height=2.5cm]{logikgatter_uebung_oder}
...
\begin{minipage}{0.45\textwidth}
\centering
\includegraphics[height=2.5cm]{logikgatter_uebung_und}
...
In einem Museum befindet sich ein Bewegungsmelder (siehe \autoref{figure-logikgatter-uebung-und}). Der Bewegungsmelder soll nur nachts einen Alarm auslösen. Dazu schaltet der Nachtwächter das Überwachungssystem mit einem Schalter \say{scharf}. Tragen Sie das entsprechende Logikgatter für diese Situation ein.
\end{exercise}

\begin{exercise}
Finden Sie ein Beispiel aus dem \textbf{Alltag} für das \texttt{NICHT}-Gatter. Beschreiben Sie die Situation.
...
% Wenn der Notausschalter in der Werkstatt nicht gedrückt ist, dann fliesst der Strom. Wenn der Schalter gedrückt wird, dann ist der Strom unterbrochen.
